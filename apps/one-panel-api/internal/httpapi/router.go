...
func (r *Router) routes(cfg HTTPConfig) {
	...
	r.mux.HandleFunc("/api/extension/bootstrap", r.requireAccount(r.handleExtensionBootstrap))
	...
	r.mux.HandleFunc("/api/transports", r.requireAccount(r.handleNodeTransports))
	r.mux.HandleFunc("/api/nodes/approve/", r.requireAccount(r.handleNodeApprove))

...
import { readConfig, writeConfig } from './storage.ts';
...
type UpdateChoice = 'update' | 'later' | 'ignore';

export async function maybeHandleCliUpdate(command: string, context: CliContext, currentVersion: string): Promise<boolean> {
...
function shouldOfferUpdate(command: string, context: CliContext): boolean {
...
async function latestCliVersion(): Promise<string> {
...
async function promptUpdateChoice(currentVersion: string, latestVersion: string): Promise<UpdateChoice> {
  const input = process.stdin;
  const output = process.stderr;
  const choices: Array<{ label: string; value: UpdateChoice }> = [
    { label: 'Update Now', value: 'update' },
    { label: 'Later', value: 'later' },
    { label: 'This version ignore', value: 'ignore' }
  ];
  let index = 0;
  let handler: ((chunk: string, key: { name?: string; ctrl?: boolean }) => void) | undefined;
  ...
  render();
  const selected = await new Promise<UpdateChoice>((resolve) => {
    handler = (_chunk, key) => {
...
function installVersion(version: string): Promise<number> {

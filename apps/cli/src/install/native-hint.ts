...
export function missingNativeBuildTools(): string[] {
  return nativeTools.filter((tool) => !commandExists(tool));
}

export function nativeBuildToolsHint(missing: string[]): string {
...
export async function nodePtyReady(): Promise<boolean> {
  try {
...
export async function printNativeDependencyHint(): Promise<void> {

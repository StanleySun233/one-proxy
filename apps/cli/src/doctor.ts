...
export async function probeTarget(target: string): Promise<TestResult> {
  const [config, state] = await Promise.all([readConfig(), readState()]);
  const route = resolveRoute({ config, state, target });
  const probes: ProbeResult[] = [await dnsProbe(route.host)];
...
export async function runDoctor(routeTarget = 'https://example.com'): Promise<DoctorResult> {
...
async function controlPlaneHealthCheck(controlPlaneUrl: string | undefined): Promise<DoctorCheck> {
  if (!controlPlaneUrl) {
...
function bootstrapSyncCheck(fetchedAt: string | undefined, accessPathCount: number, routeCount: number): DoctorCheck {
...
async function localPortsCheck(metadata: DaemonMetadata | null): Promise<DoctorCheck> {
  if (!metadata) {
...
async function entryNodeReachabilityCheck(route: RouteResult): Promise<DoctorCheck> {
  if (route.mode === 'direct') {
...
async function tcpProbe(name: string, host: string, port: number): Promise<ProbeResult> {
  const started = Date.now();
...
async function tcpOpen(host: string, port: number) {

...
export async function runProxyOnlyIsolatedCommand(input) {
...
export async function runProxyOnlyBestEffortCommand(input) {
...
export function isProxyIsolationUnavailable(error) {
    return typeof error === 'object' && error !== null && 'code' in error && error.code === 'PROXY_ISOLATION_REQUIRED';
}
export function proxyIsolationHelp(error) {
...
async function requireProxyIsolationSupport() {
...
function firewallRules(proxyPort, cgroupPath) {
    return [
        rule('iptables', [
            '-p', 'tcp',
...
async function spawnInCgroup(input, cgroupPath) {
...
function installSignalHandlers(childPid) {
...
async function killCgroupProcesses(cgroupPath) {
...
async function readCgroupPids(cgroupPath) {
    try {
...
//# sourceMappingURL=run-isolation.js.map
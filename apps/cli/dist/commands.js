...
import { processIsRunning, readConfig, readDaemonMetadata, readState, readTokens, writeConfig, } from "./storage.js";
...
export function writeError(error, context) {
    if (context.json) {
...
export async function statusCommand(_args, context) {
...
export async function overrideCommand(args, context) {
...
export async function routeCommand(args, context) {
    ...
    const route = await postIpc(daemon, '/v1/route', { target, protocol: parseTarget(target).protocol });
    write(context.json ? route : routeText(route), context);
}
export async function testCommand(args, context) {
    const target = args[0];
    if (!target) {
    ...
    await ensureSessionProxyBindings();
    const daemon = await readDaemonMetadata();
    const result = await postIpc(daemon, '/v1/probe', { target });
    if (context.json) {
        write(result, context);
        return;
    }
...
export async function doctor(_args, context) {

...
SELECT tenant_id, route_rule_group_id, MIN(permission), MIN(create_id), MIN(created_at)
FROM (
  SELECT trr.tenant_id, rr.group_id AS route_rule_group_id, trr.permission, trr.create_id, trr.created_at
  FROM tenant_route_rules trr
  JOIN route_rules rr ON rr.id = trr.route_rule_id
) bindings
GROUP BY tenant_id, route_rule_group_id;

...
import {RegexTesterModal} from './regex-tester-modal';
import {RoutePolicyHistory} from './route-policy-history';
import {RouteDeleteDialogs} from './route-delete-dialogs';
import {RouteGroupEditorModal} from './route-group-editor-modal';
import {RouteGroupsSection, RouteRulesSection} from './route-management-sections';
import {RoutePageActions} from './route-page-actions';
import {RouteRuleEditorModal} from './route-rule-editor-modal';
import {defaultRouteRuleGroupFormValues, RouteRuleGroupFormValues} from './route-rule-group-form';
...
type RouteManagementView = 'groups' | 'rules' | 'publish';

export function RouteManagementPage({view}: {view: RouteManagementView}) {
  ...
  const showsGroups = view === 'groups';
  const showsRules = view === 'rules';
  const showsPublish = view === 'publish';
  const {data: enums} = useQuery({queryKey: ['enums'], queryFn: () => fetchEnums(), enabled: showsRules});
  ...
  const routeGroupsQuery = useQuery({
    ...
    enabled: !!accessToken && (showsGroups || showsRules)
  });
  const routeRulesQuery = useQuery({
    ...
    enabled: !!accessToken && showsRules
  });
  ...
  const policyRevisionsQuery = useQuery({
    ...
    enabled: !!accessToken && showsPublish
  });
  ...
  const startCreate = useCallback(() => {
    const defaultGroupId = manageableGroups.some((group) => group.id === selectedGroupId) ? selectedGroupId : manageableGroups[0]?.id || '';
    setEditingRuleId('');
    setCreateOpen(true);
    form.reset({...defaultRouteRuleFormValues(), groupId: defaultGroupId});
  }, [form, manageableGroups, selectedGroupId]);
  ...
  const pageTitle = showsGroups ? t('shell.routeGroups') : showsPublish ? t('shell.routePublish') : t('shell.routeRules');
  ...
  return (
    <AuthGate>
      <ConsolePage
        actions={canWrite ? (
          <RoutePageActions
            createRuleDisabled={manageableGroups.length === 0}
            onCreateGroup={showsGroups ? startCreateGroup : undefined}
            onCreateRule={showsRules ? startCreate : undefined}
            onPublish={showsPublish ? () => publishMutation.mutate() : undefined}
      ...
      >
        ...
        {showsPublish ? <RoutePolicyHistory query={policyRevisionsQuery} revisions={policyRevisions} routesT={routesT} t={t} /> : null}

        {canWrite && showsGroups ? (

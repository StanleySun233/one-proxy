...
import type {AuditDashboard, AuditDecisionCount, AuditEvent, AuditGroup, NetworkAuditSummary, NetworkSession} from '@/lib/types/audit';
import {formatControlPlaneError, formatISODateTime} from '@/lib/presentation';

type AuditTab = 'business' | 'network';
...
function dashboardAsSummary(dashboard: AuditDashboard | undefined): NetworkAuditSummary | undefined {
  return dashboard ? {...dashboard, recentBusinessEvents: dashboard.recentBusinessEvents || []} : undefined;
}
...
export default function AuditPage() {
  ...
  const [activeTab, setActiveTab] = useState<AuditTab>('business');
  ...
  const summary = dashboardAsSummary(dashboard) || network?.summary;
  ...
  const recentBusiness = summary?.recentBusinessEvents || business?.items.slice(0, 5) || [];
  const totalSessions = network?.summary.total || sumGroups(tenantTraffic, 'count');
  const totalBytes = (summary?.bytesIn || sumGroups(tenantTraffic, 'bytesIn')) + (summary?.bytesOut || sumGroups(tenantTraffic, 'bytesOut'));

  return (
    <AuthGate>
      <ConsolePage title={auditT('title')}>
        <ConsoleFilterBar title={t('common.filter')}>
          <ConsoleFilterItem label={auditT('from')} match={t('common.equals')}>
            <input className="field-input" onChange={(event) => setFrom(event.target.value)} type="date" value={from} />
          </ConsoleFilterItem>
          <ConsoleFilterItem label={auditT('to')} match={t('common.equals')}>
            <input className="field-input" onChange={(event) => setTo(event.target.value)} type="date" value={to} />
          </ConsoleFilterItem>
        </ConsoleFilterBar>
        ...
        <section className="metrics-grid">
          <article className="metric-card panel-card">
            ...
            <strong>{businessQuery.isPending ? '-' : business?.summary.total ?? recentBusiness.length}</strong>
          </article>
          <article className="metric-card panel-card soft-card">
            <span>{auditT('networkSessions')}</span>
            <strong>{networkQuery.isPending ? '-' : totalSessions}</strong>
          </article>
        ...
        <div className="chain-list-actions" role="tablist">
          <button className={activeTab === 'business' ? 'primary-button' : 'secondary-button'} onClick={() => setActiveTab('business')} role="tab" type="button">
            {auditT('businessTab')}
          </button>
          <button className={activeTab === 'network' ? 'primary-button' : 'secondary-button'} onClick={() => setActiveTab('network')} role="tab" type="button">
            {auditT('networkTab')}
          </button>
        </div>
